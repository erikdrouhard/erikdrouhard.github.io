/* ==========================================================================
   condition-stack-demo.js — the interactive reconstruction on /work/mix-dialog/.

   Ported from the live site's work/mix-dialog/condition-stack-demo.js. The
   model, the markup it renders and the copy are unchanged; what changed is the
   lifecycle, because this site navigates with <ClientRouter />:

   - Registration happens in initConditionStackDemo(), not at import time. A
     module's top level runs once, so a define() up there would be a side
     effect the page could not sequence.
...
/* Every live instance, so stopConditionStackDemo() can reach them at
   astro:before-swap — before the browser disconnects them — and so a torn-down
   page leaves nothing pending behind. */
...
class ConditionStackDemo extends HTMLElement {
  ...
  /* Safe to call twice: removeEventListener on an unbound handler is a no-op,
     and the two timer ids are zeroed as they are cleared. */
  teardown() {
...
/* Idempotent by construction: a custom-element name can only be defined once
   per document, and once defined the browser upgrades every future
   <condition-stack-demo> on its own — including the ones ClientRouter swaps in.
   So re-running init on a later navigation must not (and does not) redefine
   anything. */
export function initConditionStackDemo() {
  if (!customElements.get("condition-stack-demo")) {
    customElements.define("condition-stack-demo", ConditionStackDemo);
  }
}

export function stopConditionStackDemo() {
  for (const instance of [...instances]) instance.teardown();
}
